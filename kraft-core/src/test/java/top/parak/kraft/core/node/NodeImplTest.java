...
public class NodeImplTest {
    ...
    @Test
    public void testAddNodeCannotCatchUp() throws InterruptedException, ExecutionException, TimeoutException {
    ...
    @Test
    public void testAddNodeAwaitPreviousGroupChange() throws ExecutionException, InterruptedException, TimeoutException {
        NodeConfig config = new NodeConfig();
        ...
        connector.reset();
        node.processRequestVoteResult(new RequestVoteResult(1, true)).get();
        cachedThreadTaskExecutor.submit(() ->
                node.addNode(new NodeEndpoint("D", "127.0.0.1", 2336)));
        connector.awaitAppendEntriesRpc();
        ...
        connector.awaitAppendEntriesRpc();
        Future<GroupConfigChangeTaskReference> future2 = cachedThreadTaskExecutor.submit(() ->
                node.addNode(new NodeEndpoint("E", "127.0.0.1", 2337)));
        connector.awaitAppendEntriesRpc();
        node.processAppendEntriesResult(new AppendEntriesResultMessage(
                new AppendEntriesResult("", 1, true),
    ...
    public void testOnReceiveRequestVoteResultLessThanHalfOfMajorCount() {
        NodeImpl node = (NodeImpl) newNodeBuilder(
                NodeId.of("A"),
                new NodeEndpoint("A", "127.0.0.1", 2333),
                new NodeEndpoint("B", "127.0.0.1", 2333),
                new NodeEndpoint("C", "127.0.0.1", 2333),
                new NodeEndpoint("D", "127.0.0.1", 2333),
                new NodeEndpoint("E", "127.0.0.1", 2333)
        ).setStore(new MemoryNodeStore(1, null))
                .build();
        node.start();
        node.electionTimeout();
    ...
    public void testOnReceiveAppendEntriesResultNodeRemoving() {
        ...
        node.electionTimeout(); // become candidate
        node.onReceiveRequestVoteResult(new RequestVoteResult(1, true)); // become leader
        GroupMember member = node.getContext().group().addNode(new NodeEndpoint("D", "127.0.0.1", 2336), 2, 0, false);
        member.replicateNow();
        member.setRemoving();
        node.onReceiveAppendEntriesResult(new AppendEntriesResultMessage(
                new AppendEntriesResult("", 1, true),
